...
void LCD_wave_display(Site_t site1,Site_t site2,Site_t site,Size_t size,uint8 *img,uint8 fazhi,uint8 fazhi2,uint8 maxval,uint16 Color)
{
    ...
    uint8 h2;
    uint16 y = site.y + size.H - 1;
    uint16 y1=site1.y+size.H - 1;
    uint16 y2=site2.y+size.H - 1;
    site1.x +=size.W;
    site2.x +=size.W;
    site.x += size.W;
    img  += (size.W - 1);
